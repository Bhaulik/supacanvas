...
const tools: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      "Render a canvas to a PNG image and return it as inline content. " +
      ...
      "what the canvas looks like without opening a browser. Useful when reporting back " +
      "what you built, or when iterating visually on a canvas. " +
      "Requires Chrome / Chromium / Brave / Edge / Arc installed locally. " +
      "Set SUPACANVAS_CHROME_PATH if not auto-detected.",
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "string" },
        ...
        content: [
          {
            type: "text",
            text:
              `Screenshot of "${canvas.meta.title}" (canvas ${canvas.meta.id}).\n` +
              `Size: ${png.length} bytes. URL: http://localhost:${cfg.port}/c/${canvas.meta.id}`,
          },
          {
            type: "image",
...
function asContentResult(value: unknown) {
  ...
  // canvas_screenshot uses this to return image + text content directly.
